...
args = {
    ...
    "end_times" : 100000,
    "resume" : None,
    "train_notes": "using skipmlp, skip at 4"
}
args = Munch(args)
...
device = torch.device('cuda:{}'.format(str(input("which device ? (＾－＾)V："))) if torch.cuda.is_available() else 'cpu')
...
from models import PotentialSkipMLP
pred_net = PotentialSkipMLP(dim_in=2*1000, dim_out = 1000).to(device).double()
if args.resume :
