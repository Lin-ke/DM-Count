...
args = {
    ...
    "opt_times" : 10,
    "end_times" : 100000,
    "resume" : "/home/home/weiyupeng/home/ym/pred_saved/pred_net1011-215546.pth",
    "train_notes": "go on train on mlp, using softplus as active function"
}
args = Munch(args)
...
device = torch.device('cuda:{}'.format(str(input("which device"))) if torch.cuda.is_available() else 'cpu')
...
class ot_solver():
    ...
    def potential_loss(self, a, b, f_pred, g_pred = None):
        ...
        loss =  -torch.mean(self.dual_value(a,b,f_sink,g_sink))  + 10*self.grad_value(a,b,f_sink, g_sink)
        return loss
    def consisloss():
        return;
...
from models import PotentialMLP
pred_net = PotentialMLP(dim_in=2*1000, dim_out = 1000, act = nn.Softplus()).to(device).double()
