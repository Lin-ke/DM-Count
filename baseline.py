...
# learning parameters
args.lr = lr = 0.0001
args.epochs =epochs = 20
args.batch_size =batch_size = 32
args.eps =eps = 0.02 # 1/50
args.M_EPS =M_EPS = 1e-16
args.max_iter = max_iter = 1000
args.err_bound = ep = 1e-8
args.uot = uot = False
args.warm_up = warm_up = True
...
# g,f,l = solver.solver(a, b,verbose=True)
# dual_value = solver.dual_value(a,b,f,g)
# otloss = solver.otvalue(f,g)
# pot = solver.potential_loss(a,b,f)
# print(l)
...
def model_(pretrained, requires_grad):
    ...
    model.fc = nn.Linear(2048, 1000,dtype=torch.float64)
    return model
...
def validate(model, dataloader, criterion, val_data, device):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader), total=int(len(val_data)/dataloader.batch_size)):
            ...
            tpk += top_k_cost(2, outputs, top_k_matrix)
...
pred_net = PotentialMLP(2*1000,1000).to(device)
pred_net.train()
pred_optim = torch.optim.Adam(pred_net.parameters(), lr = 1e-4)
