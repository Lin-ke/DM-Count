...
args = {
    ...
    "opt_times" : 10,
    "end_times" : 100000,
    "resume" : "/home/home/weiyupeng/thome/ym/pred_saved/pred_net1205-212744.pth",
    "convexify_iter" : 1,
    "first_save_epoch" : 40,
    "train_notes": "继续train iocnn，加入convexify, act = leakyrelu(), pred = pred-mean()"
}
args = Munch(args)
...
def train(model, dataloader, optimizer, criterion, train_data, device):
    ...
    for i, data in loop:
        ...
        #
        if counter1 % args.convexify_iter == 0:
            pred_net.convexify()
            logger.info("convexify")
        # real_f detach
...
from models import OriDenseICNN
# pred_net = OriDenseICNN(in_dim=2*1000, hidden_layer_sizes=[32,32,32], rank=1, activation='celu',  strong_convexity=1e-4,device = device).to(device).double()
from models import IOCNN
pred_net = IOCNN(dim_in=2*1000, act = nn.ReLU(), dim_out =1,device = device).to(device).double()
...
for epoch in range(epochs):
    ...
    if args.first_save_epoch is not None and epoch == args.first_save_epoch:
        torch.save({
            'epoch': epoch,
