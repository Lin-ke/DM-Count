...
args = {
    ...
    "opt_intervals" : 5,
    "opt_totals" : 3000,
    "wanted" : ["vgg", "resnet", "random", "gauss", "vit"],
    "losses" : ["pot", "grad", "reg"],
    "root_dir" : "./generated",
...
class TensorDataset(Dataset):
    def __init__(self, include_list, root_path ) -> None:
        super().__init__()
        ...
        self.tensor_names = all_tensors_list
    def __len__(self):
...
for epoch in range(epochs):
    ...
    for i, tensorset in tqdm(enumerate(train_loader), total = int(len(dst)/batch_size)):
        ...
        pred_net.eval()
        if total_count > opt_totals and total_count % opt_intervals != 0:
