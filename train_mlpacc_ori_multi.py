...
args = {
    ...
    "end_times" : 100000,
    "loss_lambda" : 2,
    "resume" : "/home/home/weiyupeng/home/ym/pred_saved/pred_net1122-210915.pth",
    "train_notes": "go on train on mlp, 3*1024, batchsize"
}
args = Munch(args)
...
class uot_solver():
    ...
    def potential_loss(self, a, b, f_pred):
        ...
        norm2 = torch.norm(gradg,dim = 1,keepdim=True).mean()
        loss =  - torch.mean(dual_value) + args.loss_lambda * norm2
        return loss
    # make prob nozero before solve
    def solver(self, a,b, max_iter = 1000,ep = 1e-8, init = None,verbose = False, print_interval = 10):
        if verbose:
...
pred_net = PotentialMLP(dim_in=2*1000, dim_out = 1000).to(device).double()
