...
ckpt_path = "/home/home/weiyupeng/thome/ym/ckpt/1130-163712"
time_str_ = ckpt_path.split("/")[-1] if ckpt_path else None
args = { 
    "save_root" : "./ckpt",
    "lr" : 0.00001,
    "epochs" : 100,
    ...
    "warm_up" : True,
    "save_dir" : ckpt_path,
    "opt_interval" : 1000000000,
    "opt_times" : 10,
    "end_times" : 100000,
    "resume" : "/home/home/weiyupeng/thome/ym/pred_saved/pred_net{}.pth".format(time_str_) if time_str_ else None,
    "train_notes": "load, no train"
}
args = Munch(args)
...
def train(model, dataloader, optimizer, criterion, train_data, device):
    ...
    for i, data in loop:
        ...
        a_dens = outputs_.requires_grad(False)
        b_dens = target.requires_grad(False)
...
pred_net = OriDenseICNN(in_dim=2*1000, hidden_layer_sizes=[512,512,512], rank=1, activation='leakyrelu',  strong_convexity=1e-2,device = device).to(device).double()

if args.resume :
    pred_net.load_state_dict(torch.load(args.resume,map_location=device)['model_state_dict'])
    logger.info("loaded")
pred_net.val()
