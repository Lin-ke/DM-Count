...
args = {
    ...
    "opt_times" : 1,
    "end_times" : 100000,
    "resume" : "/home/home/weiyupeng/home/ym/pred_saved/pred_net.pth",
    "train_notes": "一个一个来的,GT",
    "device" : "6",
    "use_init" : False,
...
C = torch.asarray(scipy.io.loadmat("/home/home/weiyupeng/home/ym/ground-metric.mat")["M"]).to(device)
K = torch.exp(-C/eps)
top_k_matrix = C 
...
train_mat = scipy.io.loadmat("/home/home/weiyupeng/home/ym/new_train_label.mat")
val_mat = scipy.io.loadmat("/home/home/weiyupeng/home/ym/new_val_label.mat")
test_mat = scipy.io.loadmat("/home/home/weiyupeng/home/ym/new_test_label.mat")
# train dataset
train_data = ImageDataset(
    train_mat, method = "train", root_path = "/home/home/weiyupeng/home/ym/train"
)
# validation dataset
valid_data = ImageDataset(
    val_mat, method = "val", root_path = "/home/home/weiyupeng/home/ym/val"
)
# train data loader
...
def train(model, dataloader, optimizer, criterion, train_data, device):
    ...
    for i, data in loop:
        ...
        pred_f = pred_net(a_dens, b_dens) # 这个可以直接用多batch的 
        # 对每个元素执行：计算loss+ploss，对每个times执行：更新pred_net
        for i in range(labels.size(0)):
            ...
            # optim pred_net
            # 这种方式只能优化1次。对每个元素使用ploss和backward代价太大。
            if counter1 % opt_interval == 0 and counter1 < end_times:
                    # (opt_times = 1)
                    ploss += criterion.potential_loss(a_dens[i].unsqueeze(0), target_density,pred_f[i].unsqueeze(0))

            if args.use_init:   
                loss += alpha*criterion(outputs[i].unsqueeze(0), target_density,f = pred_f[i].unsqueeze(0))
            else:
                loss += alpha*criterion(outputs[i].unsqueeze(0), target_density) # ground truth.
        if counter1 % opt_interval == 0 and counter1 < end_times:    
            pred_optim.zero_grad()
            ploss.backward()
            pred_optim.step()
        end_time = time.time()
        logger.info("opt time + calculate loss time (needed):{}".format(end_time - start_time))

        loss += kl_criterion(torch.log(outputs), b_dens) # 这个可以直接用多batch的
...
pred_net = PotentialMLP(2*1000,1000,3,1024).to(device)
if args.resume :
    pred_net.load_state_dict(torch.load(args.resume)['model_state_dict'])
pred_net.train()
...
torch.save({
            ...
            'loss': criterion,
            }, './pred_saved/pred_net{}.pth'.format(datetime.strftime(datetime.now(), '%m%d-%H%M%S')))
